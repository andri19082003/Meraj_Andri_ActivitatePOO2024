...
public:
	...
	Tir(const char* tara, float tonaj, string marca, int nrRoti, float pret, string* producatori, int an) :Masina(nrRoti, marca, pret, an) {
		this->tonaj = tonaj;
		this->tara = new char[strlen(tara) + 1];
		strcpy_s(this->tara, strlen(tara) + 1, tara);
	...
	Tir(const Tir& t) :Masina(t) {
		this->tonaj = t.tonaj;
		this->tara = new char[strlen(t.tara) + 1];
		strcpy_s(this->tara, strlen(t.tara) + 1, t.tara);
	}
...
int main() {
	...
	Tir t3("Romania", 15.2f, "Dac", numarRoti, 50000, prod, 2021);
	Tir t4 = t3;
	Tir t5;
	t5 = t3;
	cout << t3;
